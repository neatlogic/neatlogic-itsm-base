...
public enum ProcessTaskOperationType implements IOperationType {
    ...
    PROCESSTASK_START("startprocess", new I18n("common.report")),
    PROCESSTASK_WORK("work", new I18n("common.handle")),//可处理步骤权限，有accept、start、complete、startprocess、completetask其中一个权限，就会有work权限
    /**
     * 取消
     ...
     */
    PROCESSTASK_ABORT("abortprocessTask", new I18n("common.cancel"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.OWNER.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    /**
     * 恢复
     * 该功能是工单级别的操作，但将权限下放到步骤中配置；被授权的对象可在该步骤处于‘待处理’或‘进行中’时取消/恢复工单，如果同时有多个“待处理”或‘进行中’的步骤，则将每个步骤中配置的授权对象取并集；
     * 如果是步骤处理人操作，会记录在步骤中，同时生成一条活动；如果是非处理人操作，则步骤中无需记录，只生成活动即可；
     */
    PROCESSTASK_RECOVER("recoverprocessTask", new I18n("common.recover")),
    /**
     * 转交
     * 只有该节点处于‘待处理’或‘进行中’状态时，才允许转交；无论是查看页面还是处理页面，处理人还是非处理人，只要用户有权限转交，便能看到该按钮，否则按钮不显示；
     * 如果是步骤处理人操作，会记录在步骤中，同时生成一条活动；如果是非处理人操作，则步骤中无需记录，只生成活动即可；
     */
    PROCESSTASK_TRANSFER("transfer", new I18n("common.forward"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.MAJOR.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    ...
    PROCESSTASK_URGE("urge", new I18n("common.urgent"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.OWNER.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    PROCESSTASK_RETREAT("retreat", new I18n("common.retreat"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.MAJOR.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),

    PROCESSTASK_REDO("redo", new I18n("common.back")),
    PROCESSTASK_SCORE("score", new I18n("common.score")),
    PROCESSTASK_SHOW("show", new I18n("enum.process.processtaskoperationtype.processtask_show")),
    PROCESSTASK_HIDE("hide", new I18n("common.hidden")),
    PROCESSTASK_DELETE("delete", new I18n("common.delete")),
    PROCESSTASK_TRANSFERREPORT("tranferreport", new I18n("common.transferreport")),
    ...
    PROCESSTASK_FOCUSUSER_UPDATE("updatefocususer", new I18n("common.modifyworkorderfocus")),
    /**
     * 查看节点信息
    ...
    STEP_START("start", new I18n("common.start")),
    STEP_ACTIVE("active", new I18n("common.active")),
    STEP_COMPLETE("complete", new I18n("common.flow")),//下一步骤列表中有未激活的步骤时，有流转权限
    STEP_ACCEPT("accept", new I18n("common.start")),
    STEP_WORK("workcurrentstep", new I18n("enum.process.processtaskoperationtype.step_work")),
    STEP_TRANSFER("transfercurrentstep", new I18n("common.forward"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.MAJOR.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    STEP_BACK("back", new I18n("common.back")),//下一步骤列表中有已激活的步骤时，有回退权限
    STEP_SAVE("save", new I18n("enum.process.processtaskoperationtype.step_save")),
    STEP_REAPPROVAL("reapproval", new I18n("common.review")),
    /**
     * 修改上报内容
     * 被授权的对象可在该步骤处于‘待处理’或‘进行中’时修改上报内容，如果同时有多个“待处理”或‘进行中’的步骤，则将每个步骤中配置的授权对象取并集；
     * 如果是步骤处理人操作，会记录在步骤中，同时生成一条活动；如果是非处理人操作，则步骤中无需记录，只生成活动即可；
     */
    STEP_COMMENT("comment", new I18n("common.reply")),
    STEP_EDITCOMMENT("editcomment", new I18n("common.editreply")),
    STEP_DELETECOMMENT("deletecomment", new I18n("common.deletereply")),
    /**
     * 催办
    ...
    STEP_RECOVER("recover", new I18n("common.recover"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.MAJOR.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    STEP_RESTFULACTION("restfulaction", new I18n("common.restfulaction")),
    STEP_RETREAT("retreatcurrentstep",  new I18n("common.retreat"), Arrays.asList(ProcessTaskGroupSearch.PROCESSUSERTYPE.getValuePlugin() + ProcessUserType.MAJOR.getValue()), Arrays.asList(GroupSearch.COMMON.getValue(), ProcessTaskGroupSearch.PROCESSUSERTYPE.getValue(), GroupSearch.USER.getValue(), GroupSearch.TEAM.getValue(), GroupSearch.ROLE.getValue())),
    STEP_HANG("hang", new I18n("enum.process.processtaskoperationtype.step_hang")),
    STEP_HANDLE("handle", new I18n("common.automaticprocessing")),
