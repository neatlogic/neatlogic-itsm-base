    ...
    public void actionDispatcherInit() {
        configMap.put(TaskConfigPolicy.ALL.getValue(), (processTaskStepTaskVo) -> {
            ...
            if (CollectionUtils.isNotEmpty(processTaskStepTaskVo.getStepTaskUserVoList())) {
                if(processTaskStepTaskVo.getStepTaskUserVoList().stream().anyMatch(t->!Objects.equals(t.getStatus(), ProcessTaskStepStatus.SUCCEED.getValue()))){
                    result = false;
                }
            }
            return result;
        ...
        configMap.put(TaskConfigPolicy.ANY.getValue(), (processTaskStepVo) -> {
            ...
            if (CollectionUtils.isNotEmpty(processTaskStepVo.getStepTaskUserVoList())) {
                if(processTaskStepVo.getStepTaskUserVoList().stream().anyMatch(t->Objects.equals(t.getStatus(), ProcessTaskStepStatus.SUCCEED.getValue()))){
                    result = true;
                }
            }
            return result;
